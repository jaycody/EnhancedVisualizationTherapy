class FlowField {
  ...
  int fps=30;
  float predsec=0.5; // prediction time (sec): larger for longer vector
  ...
  FlowField(int r) {
    ...
    cols = wscreen/resolution;
    rows = hscreen/resolution;
  ...
  void difT(){
    for(int ix=0;ix<cols;ix++) {
      ...
      for(int iy=0;iy<rows;iy++) {
        ...
        pixave(x0-avSize,y0-avSize,x0+avSize,y0+avSize);
