...
void draw() {
  ...
  if (mousePressed) {
    ...
    for (int i = 0; i <= numPoints; i++) {
      float x = ((float) i)/numPoints * mouseX + (1 - ((float) i)/numPoints) * pmouseX;
      float y = ((float) i)/numPoints * mouseY + (1 - ((float) i)/numPoints) * pmouseY;
      setField(x, y, xField, constrain((mouseX-pmouseX)/10,-10,10));
      setField(x, y, yField, constrain((mouseY-pmouseY)/10,-10,10));
    }
  }
