...
void setup() {
  ...
  physics.setDrag(0.05f);  //(0.05f) = 10% drag
  physics.setWorldBounds(new Rect(0, 0, width, height));
  // the NEW way to add gravity to the simulation, using behaviors
  physics.addBehavior(new GravityBehavior(new Vec2D(.9, 0f))); //original::new Vec2D(0, 0.15f)));
}
...
void addParticle() {
  ...
  physics.addBehavior(new AttractionBehavior(p, 20, -5.5f, .010f));// original (p, 20, -1.2f, 0.01f));
}

void draw() {
  background(0,0,0);//red is original
  noStroke();
  fill(255);
