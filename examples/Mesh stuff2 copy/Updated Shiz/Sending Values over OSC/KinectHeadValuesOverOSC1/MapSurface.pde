public class CornerPinSurface implements Draggable {
  ...
  public void render(PGraphics g, PImage texture) {
    g.pushMatrix();
    g.translate(x, y);
    if (calibrate) g.stroke(gridColor);
    else g.noStroke();
    g.fill(255);
    ...
    if (calibrate) renderControlPoints(g);
  ...
  public PVector getTransformedPassedValues (int px, int py) {
    ...
    // local mouse x and y -- remove translation offset
    int lmx = passedX - (int)x;
    int lmy = passedY - (int)y;
  ...
  void load(float thisX, float thisY, int pntOneI, float pntOneX, float pntOneY, float pntOneU, float pntOneV, 
  int pntTwoI, float pntTwoX, float pntTwoY, float pntTwoU, float pntTwoV, 
  int pntThreeI, float pntThreeX, float pntThreeY, float pntThreeU, float pntThreeV, 
  int pntFourI, float pntFourX, float pntFourY, float pntFourU, float pntFourV) {

    this.x =thisX;
    this.y = thisY;

    MeshPoint mp0 = mesh[pntOneI];
    mp0.x = pntOneX;
    mp0.y = pntOneY;
    mp0.u = pntOneU;
    mp0.v = pntOneV;
    mp0.setControlPoint(true);

    MeshPoint mp1 = mesh[pntTwoI];
    mp1.x = pntTwoX;
    mp1.y = pntTwoY;
    mp1.u = pntTwoU;
    mp1.v = pntTwoV;
    mp1.setControlPoint(true);

    MeshPoint mp2 = mesh[pntThreeI];
    mp2.x = pntThreeX;
    mp2.y = pntThreeY;
    mp2.u = pntThreeU;
    mp2.v = pntThreeV;
    mp2.setControlPoint(true);

    MeshPoint mp3 = mesh[pntFourI];
    mp3.x = pntFourX;
    mp3.y = pntFourY;
    mp3.u = pntFourU;
    mp3.v = pntFourV;
    mp3.setControlPoint(true);
