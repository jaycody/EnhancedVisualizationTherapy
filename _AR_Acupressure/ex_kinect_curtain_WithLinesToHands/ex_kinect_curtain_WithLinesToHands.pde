...
float[] depthLookUp = new float[5048];
 ...
 // rotateY(-.5);
  ...
  for(int x=0; x<w-skip; x+=skip) {
    ...
    for(int y=0; y<h-skip; y+=skip) {
      ...
      if(rawDepth > 700 && rawDepth < lowerThreshold) {
        rawDepth = 700;
      }
       
      if(rawDepth2 > 700 && rawDepth2 < lowerThreshold) {//was 2040
        rawDepth2 = 700;
      }
       
      if(rawDepth3 > 700 && rawDepth3 < lowerThreshold) {
        rawDepth3 = 700;
      }
       
      if(rawDepth4 > 700 && rawDepth4 < lowerThreshold) {
        rawDepth4 = 700;
      }
