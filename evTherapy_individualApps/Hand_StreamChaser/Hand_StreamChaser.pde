...
int DisplayStep=1;// draw every Xth point. Drawing fewer points makes it go faster and lets you see the dynamics more clearly.
int NumColors=20;
float Visibility=1; // 0 to 1 transparency of the input box
int LastMouseOver=millis();// timer for clearing the input box
int BarFadeSeconds=2;// how long it takes input box to dissappear
PFont BarFont;
boolean MouseChaseOn=false;
...
void setup() //////////////////////////// Setup
{
  ...
  stroke (255,150,150);
  fill(40,40,230,200);
  //BarFont= loadFont("Verdana-48.vlw");
  //textFont(BarFont,15);
   
   
 
  LineRatioBar= new SimpleBar(10,0,300,30,.5, "Stream Effect");// create new instances of scrollbars
  WaveRangeBar= new SimpleBar(10,35,300,30,.7, "Amplitude");
  SpeedBar= new SimpleBar(10,70,300,30,.5, "Speed");
  FrequencyBar= new SimpleBar(10,105,300,30,.5, "Frequency");
  MouseRatioBar= new SimpleBar(10,140,300,30,.5, "Mouse Effect (press m)");
  initializePositions();
}
void draw()///////////////////////////// Draw
{
...
void calcPositions()///////////////////  Engine
{
  ...
  for (int j=0;j<numLines;j++)
  {
    ...
    for (int i=0; i<numPoints; i++) //calculate the directions from each point to the next
    {
      ...
      direction[j][i]= atan2(YD,XD) + WaveRange*sin(((i*TWO_PI*numWaves)/numPoints)+Phase);
    }
  }
  for (int j=0;j<numLines;j++)
  {
...
void DisplayData(){
  ...
  textAlign(LEFT);
  text ("Number of Waves "+nf(numWaves,2,0),10,190);
}
